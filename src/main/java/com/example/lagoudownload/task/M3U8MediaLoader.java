...
public class M3U8MediaLoader implements NamedTask, MediaLoader {
    ...
    public M3U8MediaLoader(String m3u8Url, String fileName, String savePath, String fileId, String cookie) {
        this.url = m3u8Url;
    ...
    private boolean tryGetVideoKey() throws IOException {
        ...
        if (null != keyUrlPath) {
            ...
            try {
                byte[] bytes = HttpUtils.getContentWithCookie(keyUrlPath, cookie);
                this.key = bytes;
                if (bytes.length > 0) {
