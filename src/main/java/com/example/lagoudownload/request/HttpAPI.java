package com.example.lagoudownload.request;
...
import com.example.lagoudownload.alibaba.AliyunApiUtils;
import com.example.lagoudownload.domain.*;
...
public class HttpAPI {
    ...
    public static CourseInfo getCourseInfo(String courseId, String cookie) {
        String url = MessageFormat.format(COURSE_INFO_API, courseId);
        log.debug("获取课程:{}信息，url：{}", courseId, url);
        HttpRequest httpRequest = HttpUtils.get(url, cookie).header("x-l-req-header", "{deviceType:1}");
     ...
     * @return
     */
    public static List<CourseCommentListInfo.CourseCommentList> getCourseCommentList(String courseId, String lessonId, String cookie) {
    ...
    public static PlayHistory getPlayHistory(String lessonId, String cookie) {
    ...
    public static CourseLessonDetail getCourseLessonDetail(String lessonId, String lessonName, String cookie) {
        String url = MessageFormat.format(COURSE_DETAIL_API, lessonId);
        String videoJson = HttpUtils.get(url, cookie).header("x-l-req-header", "{deviceType:1}").execute().body();
    ...
    public static String tryGetPlayUrlFromKaiwu(String fileId, String cookie) {
        String url = MessageFormat.format(PLAY_INFO_API, fileId);
        String body = HttpUtils.get(url, cookie).header("x-l-req-header", "{deviceType:1}").execute().body();
        JSONObject jsonObject = JSON.parseObject(body);
     ...
     * @return
     */
    public static PurchasedCourseRecord getPurchasedCourseRecord(String cookie) {
